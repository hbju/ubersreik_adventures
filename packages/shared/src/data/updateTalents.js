...
// Import the effects mapping
const talentEffectsMapping = {
  // Combat Talents
  "accurate-shot": [
...
const talentsPath = path.join(__dirname, 'talents.json');
...
try {
  ...
  // Update each talent with effects if defined in our mapping
  talentsData.forEach(talent => {
    if (talentEffectsMapping[talent.id]) {
      talent.effects = talentEffectsMapping[talent.id];
      updatedCount++;
    }
  });
  
  // Write the updated data back to the file
  fs.writeFileSync(talentsPath, JSON.stringify(talentsData, null, 4));
  
  console.log(`Successfully updated ${updatedCount} talents with effects!`);
} catch (error) {
